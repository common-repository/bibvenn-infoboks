...
Description: Lager en knapp i redigeringsskjermen som lar deg sette inn info om et objekt (tittel, forfatter, ISBN...) bare ved å oppgi ISBN eller URN hos NB
Version: 1.0.2
...
function infoboks_get_content($url) { 
...
if ($id != "") {
	...
	if (strtolower(substr($id, 0, 4) == "http")) {
		...
		$xmlfil = infoboks_get_content($rawurl);
...
$childxml = infoboks_get_content ($childxmlfil);
...
if ($nb->digital == "true") {
	...
	$structxml = infoboks_get_content ($structxmlfil);
...
// FINNE OMSLAG
	if (stristr($contentclasses, "public") || stristr($contentclasses, "bokhylla")) {
		$omslag = "http://www.nb.no/services/image/resolver/" . $urn . "_C1/full/full/0/native.jpg";
	//	$omslag = "http://www.nb.no/services/iiif/api/" . $sideid . "/full/300,/0/native.jpg";

//http://www.nb.no/services/image/resolver/URN:NBN:no-nb_digibok_2011062806089_C1/full/full/0/native.jpg
